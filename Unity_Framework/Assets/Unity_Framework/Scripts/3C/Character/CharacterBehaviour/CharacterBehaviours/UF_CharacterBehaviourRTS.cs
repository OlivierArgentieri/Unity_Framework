...
namespace uf
{
    public class UF_CharacterBehaviourRTS : UF_CharacterBehaviour, IIsValid, IEnable
    {
        ...
        [SerializeField, Range(0, 10), Header("Stop Distance ")]
        private float stopDistance = 1f;

        [SerializeField, Range(0, 500), Header("Rotate Speed")]
        private float rotateSpeed = 400;

        [SerializeField, Header("Navigable Layer")]
        private LayerMask navigableLayer = 0;

        [SerializeField, Header("Obstacle Layer")]
        private LayerMask obstacleLayer = 0;

        [SerializeField, Header("ID Main Camera")]
        private int idMainCamera = 0;
        ...
        {
            if (!IsMoving) return;
            transform.position = Vector3.MoveTowards(transform.position, _target + transform.localScale / 2,
        ...
        void RotateTo(Vector3 _target, float _rotateSpeed)
        {
            ...
            Quaternion lookAt =
                Quaternion.LookRotation((_target + transform.localScale / 2) - transform.position, transform.up);
            if ((lookAt.eulerAngles - Vector3.zero).magnitude > 0)
                transform.rotation = Quaternion.Slerp(transform.rotation,
                    Quaternion.AngleAxis(lookAt.eulerAngles.y, Vector3.up), Time.deltaTime * _rotateSpeed);
        }
