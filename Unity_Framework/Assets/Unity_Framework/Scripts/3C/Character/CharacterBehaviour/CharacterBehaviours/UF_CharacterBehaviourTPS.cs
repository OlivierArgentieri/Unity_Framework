...
namespace uf
{
    public class UF_CharacterBehaviourTPS : UF_CharacterBehaviour, IIsValid, IEnable
    {
        ...
        void OnMoveTPS(Vector2 _moveAxis)
        {
            ...
            CharacterSettings.LocalPlayer.transform.rotation =
                Quaternion.AngleAxis(mainCamera.transform.eulerAngles.y, Vector3.up);

            transform.position += _moveAxis.y * CharacterSettings.MoveSpeed * Time.deltaTime *
                                  CharacterSettings.LocalPlayer.transform.forward;
            transform.position += _moveAxis.x * CharacterSettings.MoveSpeed * Time.deltaTime *
                                  CharacterSettings.LocalPlayer.transform.right;
        }
